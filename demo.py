...
with gr.Blocks(
    ...
    with gr.Row():
        ...
        with gr.Column():
            gr.HTML(
                "<a href='https://ko-fi.com/rejekts' target='_blank'><img src='file/kofi_button.png' alt='🤝 Support Me'></a>"
            )
    with gr.Tabs():
            ...
            with gr.Row():
                with gr.Column():
                    with gr.Tabs():
                        ...
                        with gr.TabItem(
                            "TTS (experimental)",
                            visible=False if whisperspeak_on is None else True,
                        ):
                            with gr.Row():
                                tts_text = gr.Textbox(
                ...
                with gr.Column():
                    ...
                    with gr.Accordion("Processing Tools (Experimental)", open=True):
                        ...
                        with gr.Column():
                            sr_button = gr.Button(
                                value="SuperResolution", variant="primary"
                            )
                            sr_button.click(
                                fn=sr_process,
                                inputs=[input_player, output_player, audio_choice],
    ...
    if config.iscolab:
        app.queue(max_size=20).launch(
            share=True, allowed_paths=["a.png", "kofi_button.png"], show_error=True
        )
    else:
